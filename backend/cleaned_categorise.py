...
Workflow (no args):
  ...
  - For each file:
      ...
      * NEW: summarize description into `description_summarized` (100–500 words, one paragraph, key info incl. salary)
      * NEW: assign `job_category`
      * write two outputs to OUTPUT_DIR (default: ./job-board/public):
          - <name>_labeled.json (all records with labels + possibly enriched apply_link + likely_to_sponsor + summary + category)
...
# =========================
CATEGORIES = [
    "Tech", "Finance", "HR", "Healthcare", "Engineering", "Data", "Product",
    "Sales", "Marketing", "Operations", "Legal", "Education",
    "Customer Support", "Design", "Other"
]

def get_company(rec: Dict[str, Any]) -> str:
    return str(rec.get("company_name") or rec.get("employer") or "").strip()

def get_location(rec: Dict[str, Any]) -> str:
    di = rec.get("discovery_input") or {}
    if isinstance(di, dict):
        return str(di.get("location") or "").strip()
    return ""

def call_openai_summary_and_category(title: str, company: str, location: str, salary: str, description: str) -> Tuple[str, str]:
    """
    Uses OpenAI to produce:
      - summary: one cohesive paragraph, 100–500 words, covering key duties, skills, salary (if present), location/arrangements,
                 contract/seniority, team/context, and other relevant details. No fluff, no invented facts.
      - job_category: one value from CATEGORIES.
    """
    if not OPENAI_API_KEY:
        raise RuntimeError("OPENAI_API_KEY is not set")
    headers = {"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"}
    system = (
        "You help extract structured information from job adverts.\n"
        "Task 1 — SUMMARY: Write ONE cohesive paragraph between 100 and 500 words that captures the essential details "
        "of the job: core purpose of the role, key responsibilities/duties, required skills/technologies/qualifications, "
        "seniority/experience level, contract type (if given), working pattern/location (onsite/remote/hybrid), "
        "team or reporting context, benefits/perks if notable, and the stated salary or salary range if present. "
        "Keep it factual and concise; do not invent details; use only information present in the input fields.\n"
        f"Task 2 — CATEGORY: Pick exactly ONE category from this list: {', '.join(CATEGORIES)}. "
        "Base it on the title and description (e.g., Software Engineer→Tech; Data Analyst→Data; Nurse→Healthcare; "
        "Accountant→Finance; HR Business Partner→HR; Teacher→Education). If nothing fits, return 'Other'.\n"
        "Respond strictly as JSON with keys: summary, job_category."
    )
    user = {
        "job_title": title[:200],
        "company_name": company[:200],
        "location": location[:200],
        "salary_formatted": salary[:200],
    ...
    category = str(obj.get("job_category", "")).strip()
    if category not in CATEGORIES:
        category = "Other"
    return summary, category

# Fallbacks used only if the API call fails, to keep the pipeline running
def simple_summary(text: str, min_words: int = 100, max_words: int = 500) -> str:
    if not text:
        return ""
    sentences = [s.strip() for s in SENT_SPLIT.split(text) if s.strip()]
    words = []
...
CATEGORY_KEYWORDS = [
    ("Tech", r"developer|software|frontend|backend|full\s*stack|devops|sre|cloud|python|java(script)?|react|node|typescript|golang|qa|test(ing)?|mobile|ios|android|kotlin|swift"),
    ("Data", r"data\s+(scientist|engineer|analyst)|machine\s*learning|ml|ai|business\s+intelligence|bi|analytics|etl|dbt|sql|power\s*bi|tableau"),
    ("Product", r"product\s+(manager|owner)|\bpm\b"),
    ("Design", r"\bux\b|\bui\b|designer|product\s+design|graphic\s+design"),
    ("Finance", r"accountant|accounting|finance|financial|fp&a|audit|auditor|acca|aca|cima|tax|payroll|treasury"),
    ("HR", r"\bhr\b|human\s+resources|talent\s+acquisition|recruit(er|ing)|people\s+partner"),
    ("Healthcare", r"nurse|doctor|gp|clinician|clinical|nhs|pharmac(y|ist)|radiograph|physio|therap"),
    ("Sales", r"sales|account\s+executive|bd[rm]|business\s+development|salesforce|account\s+manager"),
    ("Marketing", r"marketing|seo|ppc|paid\s+(search|social)|growth|content\s+marketing|copywriter|brand|campaign"),
    ("Operations", r"operations|supply\s+chain|logistics|warehouse|fleet|procurement|purchas|op(eration)?s\s+manager"),
    ("Legal", r"solicitor|lawyer|legal|paralegal|compliance|gc\b|counsel"),
    ("Education", r"teacher|lecturer|tutor|teaching\s+assistant|school|university|curriculum|ofsted"),
    ("Customer Support", r"customer\s+(support|service)|helpdesk|call\s+center|contact\s+centre|technical\s+support"),
    ("Engineering", r"mechanical|electrical|civil|manufacturing|industrial|process\s+engineer|cad|autocad"),
]

def keyword_category(title: str, description: str) -> str:
...
def classify_record(rec: Dict[str, Any], desc_key: Optional[str]) -> Dict[str, Any]:
    ...
    # Likely-to-sponsor rating for YES/Maybe
    ...
    # ===== NEW: Summary + Category =====
    company = get_company(rec)
    location = get_location(rec)
    try:
        summary, category = call_openai_summary_and_category(title, company, location, salary_str, desc or combined)
    except Exception:
        summary = simple_summary(desc or combined, 100, 500)
        category = keyword_category(title, desc or combined)
    out["description_summarized"] = summary
    out["job_category"] = category
    # ===== END NEW =====
...
def process_file(path: Path, out_dir: Path) -> Dict[str, Any]:
    ...
    # Classify first (now uses title + description; adds likely_to_sponsor, summary, category)
...
def main():
    parser = argparse.ArgumentParser(description="Batch classify visa sponsorship, enrich apply_link, summarize/category, and filter out 'No'.")
